...
export function useUserAppIds (firebase, uid) {
  ...
  return useSelector(({ firebase }) => {
    if (firebase.ordered.user_apps && firebase.ordered.user_apps.by_user_id && firebase.ordered.user_apps.by_user_id[uid]) {
...
export function useUserAppsLoading (uid) {
  return useSelector(state => state.firebase.requesting[`user_apps/by_user_id/${uid}`])
}
...
export function useUserAppLinkId (firebase, uid, appId) {
  ...
  return useSelector(({ firebase }) => {
    if (firebase.ordered.user_apps && firebase.ordered.user_apps.by_user_id && firebase.ordered.user_apps.by_user_id[uid]) {
      const appLink = firebase.ordered.user_apps.by_user_id[uid].find(({ value }) => {
