...
const SettingsLink = React.forwardRef<HTMLElement, ComponentProps<any>>((props, ref) => (
  <RouterLink to="/settings" innerRef={ref} {...props} />
))

const InputSettings: FunctionComponent = () => {
  const dispatch = useDispatch()

  const nrmlvoEntries: nrmlvo[] = useSelector((state) => state.compositor.seat.keyboard.nrmlvoEntries)
  const defaultNrmlvo = useSelector((state) => state.compositor.seat.keyboard.defaultNrmlvo)
  ...
  const keyboardLayoutName = useSelector((state: DefaultRootState) => {
    if (isInitialized) {
      return state.compositor.configuration?.keyboardLayoutName ?? defaultNrmlvo?.name ?? ''
    } else {
      return ''
    }
  })
  ...
  const scrollFactor = useSelector((state: DefaultRootState) => state.compositor.configuration?.scrollFactor ?? 1)
