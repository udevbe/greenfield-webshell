...
export function useIsAdmin(uid) {
  ...
  return useSelector((state) =>
    state.firebase.data.admins
      ? state.firebase.data.admins[uid] || false
      : false
  )
}
...
export function useUserRoles(uid) {
  ...
  return useSelector(
    (state) =>
      state.firebase.ordered.user_roles
        ? state.firebase.ordered.user_roles[uid] || []
        : [],
    shallowEqual
  )
}
...
export function useUserRoleEnabledLoading(uid, roleId) {
  return useSelector(
    (state) => state.firebase.requesting[`user_roles/${uid}/${roleId}`]
  )
}
...
export function useGrant(uid, grant) {
  ...
  return useSelector(({ firebase }) => {
    if (firebase.data.user_roles && firebase.data.user_roles[uid]) {
      ...
      const enabledUserRoles = Object.entries(userRoles).filter(
        ([_, enabled]) => enabled
      )
...
export function useIsAuthenticated() {
  return useSelector(
    ({ firebase: { auth } }) => isLoaded(auth) && !isEmpty(auth)
  )
}
